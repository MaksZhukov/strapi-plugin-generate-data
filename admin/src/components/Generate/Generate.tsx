        ...
        Object.keys(attributes)
          ...
          .forEach((key) => {
            if (attributes[key].type === AttributeType.Integer) {
              ...
              AttributeType.Richtext === attributes[key].type
            ) {
              let { count } = values[key] as { count: number };
              obj[key] = faker.random.words(count);
            } else if (attributes[key].type === AttributeType.Email) {
              obj[key] = faker.internet.email();
            } else if (attributes[key].type === AttributeType.Date) {
              let { from, to } = values[key] as {
                from: Date;
                to: Date;
              };
              obj[key] = faker.date.between(from, to);
            } else if (attributes[key].type === AttributeType.Media) {
              ...
              if (min && max) {
                obj[key] = new Array(faker.datatype.number({ min, max }))
                  .fill(null)
                  .map(() => faker.image.image(width, height, true));
              } else {
                obj[key] = [faker.image.image(width, height, true)];
              }
            } else {
